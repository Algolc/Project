    ...
    <head>
        ...
        <script type="text/javascript" language="javascript">
        	...
        	function check_login() {
        		...
        		$.post(
        			"login.do",
        			$("#myform").serialize(),//发送表单数据的简便写法(用'&'符号将各个数据以键值对方式连接起来)
        			function(data) {//adminCode=zs&password=123&code=xxxx
        				if(data.flag==1) {//账号错误
        					$("#admin_code_msg").text("账号错误.");
        				} else if(data.flag==2) {//密码错误
        					$("#password_msg").text("密码错误.");
        				}else if(data.flag==4){
        					$("#password_msg").text("用户名或密码错误.");
        				} else if(data.flag==3) {//验证码错误
        					$("#code_msg").text("验证码错误.");
        					change();
        				} else {//成功
        	...
        	<form action="login.do" method="post" id="myform">
	            <table>
	                ...
	                <tr>
	                    ...
	                    <td class="width70"><input name="code" type="text" class="width70" id="code"/></td><!-- onfocus="set_msg('code_msg','');" -->
